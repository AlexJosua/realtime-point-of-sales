...
export default function Dashboard() {
  ...
  const oneLastMonth = new Date();
  oneLastMonth.setMonth(oneLastMonth.getMonth() - 1);
  oneLastMonth.setHours(0, 0, 0, 0);
  ...
  const { data: orders } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("orders")
        .select("created_at")
        .gte("created_at", oneLastMonth.toISOString())
        .order("created_at");
  ...
  const { data: revenue } = useQuery({
    ...
    queryFn: async () => {
      ...
      const averageRevenueThisMonth =
        daysInData > 0 ? Math.round(totalRevenueThisMonth / daysInData) : 0;
  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        ...
        <Card>
          <CardHeader>
            <CardDescription>Growht Rate</CardDescription>
            <CardTitle className="font-bold text-3xl">
              {revenue?.growthRate}%
            </CardTitle>
          </CardHeader>
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        <Card className="w-full lg:w-2/3">
          <CardHeader>
            ...
            <CardDescription>
              Showing orders from {oneLastMonth.toLocaleDateString("en-US")} to{" "}
